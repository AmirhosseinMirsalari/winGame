...
function General({ user }: Props) {
  ...
  const uploadImageHandler = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      errorMessage("آپلود تصویر ناموفق بود");
      setImgLoading(false);
      console.log(err);
    }
  };
  ...
  const submitHandler = async (event: FormEvent) => {
    ...
    if (enteredPhone && enteredPhone.length < 10) {
      setPhoneErrorMessage("شماره موبایل شما باید حداقل 10 عدد باشد");
      return;
    }
    ...
    try {
      ...
      successMessage("اطلاعات شما با موفقیت آپدیت شد");
      setPhoneErrorMessage("");
    } catch (err) {
      console.log(err);
      errorMessage("بروزرسانی اطلاعات شما با خطا مواجه شد");
      setPhoneErrorMessage("");
    }
  };
  return (
      ...
      <Grid container item xs={12} md={8} spacing={3} sx={{ zIndex: "1 !important" }}>
        <Grid item xs={12}>
          <FormControl fullWidth>
            <PFormLabel>نام کاربری</PFormLabel>
            <PTextField
        ...
        <Grid item xs={12}>
          <FormControl fullWidth>
            <PFormLabel>شماره موبایل</PFormLabel>
            <PTextField
              value={enteredPhone}
          ...
          <Box sx={{ textAlign: "center", marginTop: "20px" }}>
            {imgLoading ? (
              ...
              <POutlinedButton variant="contained" onClick={() => ImageRef!.current!.click()}>
                <CloudUpload sx={{ marginRight: "5px" }} /> آپلود تصویر
              </POutlinedButton>
            )}
          </Box>
        </Box>
